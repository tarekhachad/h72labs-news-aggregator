...
4. **QA pass per feature** — after each feature (a roadmap phase, or any meaningfully-sized addition) is implemented, before considering it done: spawn the `qa` subagent (`.claude/agents/qa.md`) against the new/changed code, then the `code-reviewer` subagent (`.claude/agents/code-reviewer.md`). Both have zero context on how the feature was built or reasoned about — that's intentional, it's what makes the feedback unbiased rather than the implementer grading its own work. `qa` writes and runs real tests (or, for something better exercised live than unit-tested — like an API route or a UI flow — actually runs the app and hits it); `code-reviewer` reviews the diff itself for correctness/readability/performance/security. Fix what they find, then move on. This is a standing convention documented here, not a hook-enforced gate — it relies on actually following it every time a feature is "done," the same as any other rule in this file.
...
> **Status:** Phase 2 built and real-usage-tested — real accounts via Supabase Auth, curated topic/source selection, digest pipeline driven by real per-user profiles, with three real bugs found through Tarek's own local testing (not automated QA) and fixed same-day.
...
**Next session:** build the profile/settings page Tarek asked for (edit topics/sources reusing the onboarding form + delete-then-insert action, change password via Supabase's `updateUser`) — queued, not started. Revisit the deferred triage-concurrency and topic/source-selection-cap questions when convenient.
