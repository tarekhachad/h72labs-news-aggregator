...
Design docs are in `docs/`: `ARCHITECTURE.md`, `TECH_STACK.md`, `ROADMAP.md`, `DATABASE_SCHEMA.md`, `GLOSSARY.md`, `DATA_HANDLING.md`. Next: `git init` (+ GitHub remote), then a fresh Claude Code session in plan mode to scope Phase 1 of the roadmap (the core synthesis pipeline, hardcoded profile, no auth yet).
