...
4. **QA pass per feature, fix, then re-verify — iterate to true convergence, no self-imposed cap** — after each feature (a roadmap phase, OR any meaningfully-sized addition) is implemented, before considering it done: spawn the `qa` subagent (`.claude/agents/qa.md`) against the new/changed code, then the `code-reviewer` subagent (`.claude/agents/code-reviewer.md`). Both have zero context at spawn time on how the feature was built or reasoned about — that's intentional, it's what makes the feedback unbiased rather than the implementer grading its own work. `qa` writes and runs real tests (or, for something better exercised live than unit-tested — like an API route or a UI flow — actually runs the app and hits it); `code-reviewer` reviews the diff itself for correctness/readability/performance/security.
   ...
   **Delivery bar (enforced, checked by `qa`/`code-reviewer` — see `.claude/agents/`):**

   - A feature MUST NOT be marked done without a `qa`+`code-reviewer` round that came back with zero findings from both.
   - A round MUST run both agents, every time — never one alone, even for a narrow re-check.
   - A fix MUST get the same unbiased two-agent pass as the code it fixed — no self-verification substitute.
   - Stopping before a fully clean round MUST be a conscious decision put to Tarek — never inferred from cost, time, or "diminishing returns."
...
5. **Sync the project's outward-facing status** — once a feature/phase clears the qa/code-reviewer loop above (same trigger: a roadmap phase, OR any meaningfully-sized addition), propagate what changed to **both** places outside this repo that describe this project:
   **(a)** Run the bundled `project-resume-sync` skill (`.claude/skills/project-resume-sync.md`) to update `Job Applications/Master Resources/projects/(C) personalized-news-aggregator.md`. This is what keeps `job-application-prep`'s resume bullets and cover-letter project descriptions current — that skill no longer reads this repo directly, only the synced file, so skipping this means the next resume/cover letter built off this project goes stale.
   **(b)** Refresh this project's one-line status in the **root** `CLAUDE.md`'s *My Current Projects & Overviews* section (and its Folder Structure tree comment, if that changed too). That line is what every cross-domain Cowork session reads when it pulls this project into anything else.
   Both are the same propagation step off the same trigger, which is why they're one item. **(b) is the one that rots**, because nothing surfaces it: the root file lives outside this git repo, so no `git status` ever shows it drifting while the project moves. That is exactly what happened between 2026-08-12 and 2026-08-18 — the resume file (an enforced per-phase step) stayed current through v1 closing, while the root one-liner still described a finished v1 as "one phase from the end," because the root had owners only at scaffold, brainstorm, and ship, and none during the build. Standing convention, not hook-enforced, same as step 4.
...
## Working in Claude Code vs. Cowork
...
> **Last updated:** 2026-09-14 — **v1 closed; V2.0's option is DECIDED (Option B: Tarek's key, server-side only, hard caps) and the V2 roadmap is restructured.** Three commits pushed (`2521c2c`): the roadmap regroup, the run-shape rename, and a commenting standard plus a full cleanup against it. **Tests 746 across 58 files**, `tsc` clean, eslint 0 errors, `npm run cost-report` working. **The capping strategy gets its OWN dedicated session** (Tarek's instruction) — it is a design problem, not a number, and how many people get invited is an output of it rather than an input. **The finding that forces its shape: a narrow profile (2 topics, 3 sources) costs \$0.036677 and a full one (13 topics, 48 sources) costs \$0.471763 — 13× on the same "one digest," so a per-day digest limit bounds nothing.** **Run shapes renamed to the product's vocabulary** — `firstEver` / `firstOfDay` / `sameDayTopUp`; the old `cold`/`warmNewDay`/`warmSameDay` inverted it, and `normalizeRunShape` maps legacy rows on read because `usage_runs` has no update policy by design. **Blocking the cap sizing and still open:** `deriveRunShape` reads the cursor as null/non-null only and never parses it, so a first-of-day run after 24 hours and one after three days share a label and a mean — the effective ingest window `min(now − cursor, 48h)` and whether dedup fired are the two facts a cap actually consumes, and neither is recorded. **Two cost runs are owed:** a same-day top-up, and a first-of-day run with a ~24h-stale cursor (2026-09-14's run had a three-day gap, which saturates the 48h ceiling and costs first-ever money — \$0.471763 for 1147 articles / 87 cards, flagged `isFloor`). **Correction carried forward: "a returning daily user costs ~2× less" was never measured and the mechanism argues against it** — the cursor reduces ingestion, and ingestion is free. **Next: the capping session.** Before any V2 code, raise `ROADMAP.md` → *V2 SETUP* unprompted; note this session judged two of its three skills low-value here, but the call is Tarek's.
