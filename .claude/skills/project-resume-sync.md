...
Set `last_synced: <today's date>` and `source: <what was read through>` (e.g. `"CLAUDE.md Current Status + project-log.md through 2026-08-03"`) so future runs and other tooling can tell how fresh the entry is.
...
- **Truth only.** Every claim traces to this project's own `CLAUDE.md`, `docs/`, `project-log.md`, or `git log`. Never invent results, metrics, or scope — same standard `job-application-prep` itself holds to.
