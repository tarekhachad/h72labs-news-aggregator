...
export async function POST(
  ...
  // set_expanded_report carries both conditions this write has always needed:
  // the card's digest belongs to the caller, and expanded_report is still
  // null. The second one matters because the read-then-generate above isn't
  // atomic — two near-simultaneous requests for the same card (the client
  // already guards against this, but belt-and-suspenders) can both reach this
  // point, each having generated its own report. First writer wins, so the
  // cached text doesn't flap between two valid-but-different generations
  // depending on request timing.
  //
  // A false return is that race resolving normally, not a failure: someone
  // else's identical request cached its own report first.
  const { error: updateError } = await supabase.rpc("set_expanded_report", {
    p_card_id: id,
    p_report: expandedReport,
  });
